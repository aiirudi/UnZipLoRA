    ...
    def forward(
        ...
        Args:
            ...
            encoder_hidden_states ( `torch.FloatTensor` of shape `(batch size, sequence len, embed dims)`, *optional*):
                ...
                self-attention.
            encoder_hidden_states_content ( `torch.FloatTensor` of shape `(batch size, sequence len, embed dims)`, *optional*):
                Conditional content embeddings for cross attention layer. If not given, cross-attention defaults to
                self-attention.
            encoder_hidden_states_style ( `torch.FloatTensor` of shape `(batch size, sequence len, embed dims)`, *optional*):
                Conditional style embeddings for cross attention layer. If not given, cross-attention defaults to
                self-attention.
        ...
        # 将 1 = 保留，0 = 丢弃的 mask 转换为0=保留，-10000=丢弃的bias格式
        # 方便直接加到 attention score 上 
        ...
        if self.is_input_continuous:
            batch, _, height, width = hidden_states.shape # B, C, H, W
            residual = hidden_states # 保留残差

            # 将输入映射到能做 Attention 的形式，输入是 (B, C, H, W) 的，Attention 需要 （B, seq_len, inner_dim） 的序列格式
        ...
        if self.caption_projection is not None:
            batch_size = hidden_states.shape[0] # B
        ...
        # 经过这个 for 循环之后最后的 hidden_states 形状还是: (B, seq_len==H*W, inner_dim)
        for block in self.transformer_blocks:
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = torch.utils.checkpoint.checkpoint(
                    ...
                    encoder_hidden_states_content, #新增 encoder_hidden_states_content
                    encoder_hidden_states_style, #新增 encoder_hidden_states_style
        ...
        # 将 hidden_states从 B, H*W, inner_dim 转换回 B, C, H, W
        # 3. Output
        if self.is_input_continuous:
            if not self.use_linear_projection:
