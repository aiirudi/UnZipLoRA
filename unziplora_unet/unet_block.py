...
class CrossAttnDownBlock2D(CrossAttnDownBlock2D):
    ...
    def forward(
        ...
        temb: Optional[torch.FloatTensor] = None,
        encoder_hidden_states: Optional[torch.FloatTensor] = None,
        encoder_hidden_states_content: Optional[torch.FloatTensor] = None, # content_text_input
        encoder_hidden_states_style: Optional[torch.FloatTensor] = None, # style_text_inputs
        attention_mask: Optional[torch.FloatTensor] = None,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        encoder_attention_mask: Optional[torch.FloatTensor] = None,
        ...
        for i, (resnet, attn) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
                ...
                # 因为return_dict=False， 所以用下标 0 来取数据
                hidden_states = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    encoder_hidden_states_content=encoder_hidden_states_content,
